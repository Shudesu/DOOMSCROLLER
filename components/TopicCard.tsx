  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm p-5 hover:shadow-md transition-shadow">
      ...
      <div className="mb-3 p-3 bg-amber-50 rounded-xl border border-amber-100">
        <p className="text-xs text-amber-600 font-medium mb-1">掴みセリフ</p>
        <p className="text-sm text-gray-800 leading-relaxed">「{hook}」</p>
      </div>

      {/* Outline */}
      <div className="mb-3">
        <p className="text-xs text-gray-500 font-medium mb-1">構成</p>
      ...
      {refs.length > 0 && (
        <div className="border-t border-gray-100 pt-3">
          <p className="text-xs text-gray-500 font-medium mb-2">参考投稿</p>
          <div className="flex flex-wrap gap-2">
            {refs.map((post) => (
              <button
                key={post.ig_code}
                onClick={() => onReelClick(post.ig_code)}
                className="flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-50 hover:bg-gray-100 rounded-lg text-xs text-gray-600 hover:text-gray-900 transition-colors border border-gray-200/60"
              >
                <svg
